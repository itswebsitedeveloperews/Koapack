    ...
    data: {
      ...
      fields: {
        create: fields.map((field, index) => ({
          ...
          values: {
            create: valuesFromText(field.valuesText).map((value, valueIndex) => ({
              label: value,
              value: slugify(value),
        ...
        <s-stack direction="block" gap="base">
          ...
          <s-section heading="Option fields">
            <div style={toolbarStyle}>
              {Object.keys(FIELD_TEMPLATES).map((type) => (
                <s-button key={type} type="button" onClick={() => addField(type)}>
          ...
          <s-section heading="Product targeting">
            <div style={gridStyle}>
              ...
              <Field label="Manual product handle or GID" htmlFor="manual-product">
            ...
            {targets.length > 0 ? (
              <div style={pillListStyle}>
                {targets.map((target, index) => (
                  <span key={`${target.id || target.handle}-${index}`} style={pillStyle}>
          ...
          <s-stack direction="inline" justifyContent="end" gap="small">
            ...
            <button style={primarySubmitStyle} type="submit" disabled={isSubmitting}>
...
function OptionFieldEditor({ field, index, onChange, onRemove }) {
  const hasValues = ["size", "printing", "dropdown", "swatch", "checkbox"].includes(
    field.type,
  );
