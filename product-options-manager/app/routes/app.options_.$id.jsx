  ...
  const [fields, setFields] = useState(() =>
    isNew
      ...
      : group.fields
          ...
          .map((field, index) => createFieldFromSavedField(field, index)),
  );
  const [variationPrices, setVariationPrices] = useState(() =>
    isNew ? [] : getSavedVariationPriceRows(group.fields),
  );
...
function getSavedVariationPriceRows(fields = []) {
  const storageField = fields.find(isVariationPriceStorageField);

  if (!storageField) return [];

  const saved = parseSavedValues(storageField.valuesJson);

  return normalizeVariationPriceRows(
    saved.config?.prices || saved.config?.variationPrices || saved.prices,
  );
}
